 ...
 *
 *    $ssh = new \OCA\NetBase\Vendor\phpseclib3\Net\SSH2('www.domain.tld');
 ...
 *    include 'vendor/autoload.php';
 *
 *    $key = \OCA\NetBase\Vendor\phpseclib3\Crypt\PublicKeyLoader::load('...', '(optional) password');
 *
 *    $ssh = new \OCA\NetBase\Vendor\phpseclib3\Net\SSH2('www.domain.tld');
...
namespace OCA\NetBase\Vendor\phpseclib3\Net;

use OCA\NetBase\Vendor\phpseclib3\Common\Functions\Strings;
use OCA\NetBase\Vendor\phpseclib3\Crypt\Blowfish;
use OCA\NetBase\Vendor\phpseclib3\Crypt\ChaCha20;
use OCA\NetBase\Vendor\phpseclib3\Crypt\Common\AsymmetricKey;
use OCA\NetBase\Vendor\phpseclib3\Crypt\Common\PrivateKey;
use OCA\NetBase\Vendor\phpseclib3\Crypt\Common\PublicKey;
use OCA\NetBase\Vendor\phpseclib3\Crypt\Common\SymmetricKey;
use OCA\NetBase\Vendor\phpseclib3\Crypt\DH;
use OCA\NetBase\Vendor\phpseclib3\Crypt\DSA;
use OCA\NetBase\Vendor\phpseclib3\Crypt\EC;
use OCA\NetBase\Vendor\phpseclib3\Crypt\Hash;
use OCA\NetBase\Vendor\phpseclib3\Crypt\Random;
use OCA\NetBase\Vendor\phpseclib3\Crypt\RC4;
use OCA\NetBase\Vendor\phpseclib3\Crypt\Rijndael;
use OCA\NetBase\Vendor\phpseclib3\Crypt\RSA;
use OCA\NetBase\Vendor\phpseclib3\Crypt\TripleDES; // Used to do Diffie-Hellman key exchange and DSA/RSA signature verification.
use OCA\NetBase\Vendor\phpseclib3\Crypt\Twofish;
use OCA\NetBase\Vendor\phpseclib3\Exception\ConnectionClosedException;
use OCA\NetBase\Vendor\phpseclib3\Exception\InsufficientSetupException;
use OCA\NetBase\Vendor\phpseclib3\Exception\InvalidPacketLengthException;
use OCA\NetBase\Vendor\phpseclib3\Exception\NoSupportedAlgorithmsException;
use OCA\NetBase\Vendor\phpseclib3\Exception\TimeoutException;
use OCA\NetBase\Vendor\phpseclib3\Exception\UnableToConnectException;
use OCA\NetBase\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException;
use OCA\NetBase\Vendor\phpseclib3\Exception\UnsupportedCurveException;
use OCA\NetBase\Vendor\phpseclib3\Math\BigInteger;
use OCA\NetBase\Vendor\phpseclib3\System\SSH\Agent;
...
class SSH2
{
     ...
     * \OCA\NetBase\Vendor\phpseclib3\Crypt\Common\SymmetricKey.
     *
     * @param string $algorithm Name of the encryption algorithm
    ...
    /**
     * Maps an encryption algorithm name to an instance of a subclass of
     * \OCA\NetBase\Vendor\phpseclib3\Crypt\Hash.
     *
     * @param string $algorithm Name of the encryption algorithm
     ...
     * Login
     *
     * The $password parameter can be a plaintext password, a \OCA\NetBase\Vendor\phpseclib3\Crypt\RSA|EC|DSA object, a \OCA\NetBase\Vendor\phpseclib3\System\SSH\Agent object or an array
     *
        ...
        foreach ($args as $arg) {
            switch (true) {
                ...
                case Strings::is_stringable($arg):
                    break;
                default:
                    throw new \UnexpectedValueException('$password needs to either be an instance of \OCA\NetBase\Vendor\phpseclib3\Crypt\Common\PrivateKey, \System\SSH\Agent, an array or a string');
            }
        }
        ...
        if ($publickey instanceof RSA) {
            ...
            switch ($curveName) {
                ...
                default:
                    ...
                    throw new UnsupportedCurveException('Named Curve of ' . $curveName . ' is not supported by OCA\NetBase\Vendor\phpseclib3\'s SSH2 implementation');
            }
     ...
     * Execute Command
     *
     * If $callback is set to false then \OCA\NetBase\Vendor\phpseclib3\Net\SSH2::get_channel_packet(self::CHANNEL_EXEC) will need to be called manually.
    ...
    public function exec($command, $callback = null)
    {
        ...
        // down.  the one place where it might be desirable is if you're doing something like \OCA\NetBase\Vendor\phpseclib3\Net\SSH2::exec('ping localhost &').
     ...
     *
     * \OCA\NetBase\Vendor\phpseclib3\Net\SSH2 doesn't properly close most channels.  For exec() channels are normally closed by the server
