...
/**
 * Base Class for all \OCA\NetBase\Vendor\phpseclib3\Crypt\* cipher classes
 *
 * PHP version 5
 *
 ...
 *
 *  - The new \OCA\NetBase\Vendor\phpseclib3\Crypt\* cipher class should extend \OCA\NetBase\Vendor\phpseclib3\Crypt\Common\SymmetricKey
 *
 ...
 *
 *  - Look at the source code of the current ciphers how they extend \OCA\NetBase\Vendor\phpseclib3\Crypt\Common\SymmetricKey
...
namespace OCA\NetBase\Vendor\phpseclib3\Crypt\Common;
...
use OCA\NetBase\Vendor\phpseclib3\Exception\BadDecryptionException;
use OCA\NetBase\Vendor\phpseclib3\Exception\BadModeException;
use OCA\NetBase\Vendor\phpseclib3\Exception\InconsistentSetupException;
...
use OCA\NetBase\Vendor\phpseclib3\Math\BinaryField;
use OCA\NetBase\Vendor\phpseclib3\Math\PrimeField;
...
abstract class SymmetricKey
{
     ...
     *
     * @see \OCA\NetBase\Vendor\phpseclib3\Crypt\Twofish::setKey()
     * @see \OCA\NetBase\Vendor\phpseclib3\Crypt\Twofish::setIV()
     * @var bool
     */
     ...
     *
     * Put another way, when the continuous buffer is enabled, the state of the \OCA\NetBase\Vendor\phpseclib3\Crypt\*() object changes after each
     ...
     * Encrypts a block
     *
     * Note: Must be extended by the child \OCA\NetBase\Vendor\phpseclib3\Crypt\* class
     *
     * @param string $in
     * @return string
     */
     ...
     *
     * Note: Must extend by the child \OCA\NetBase\Vendor\phpseclib3\Crypt\* class
     *
     * @see self::setup()
     */
     ...
     *       - In case of using inline crypting, _setupInlineCrypt() must extend by the child \OCA\NetBase\Vendor\phpseclib3\Crypt\* class.
     ...
     *    </code>
     *
     *    See also the \OCA\NetBase\Vendor\phpseclib3\Crypt\*::_setupInlineCrypt()'s for
                    ...
                    if (strlen($_buffer["ciphertext"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            if (strlen($_block) > strlen($_buffer["ciphertext"])) {
                                $in = $_xor;
                                ' . $encrypt_block . '
                                \OCA\NetBase\Vendor\phpseclib3\Common\Functions\Strings::increment_str($_xor);
                                $_buffer["ciphertext"].= $in;
                            }
                            $_key = \OCA\NetBase\Vendor\phpseclib3\Common\Functions\Strings::shift($_buffer["ciphertext"], ' . $block_size . ');
                ...
                $encrypt = $init_encrypt . '
                    ...
                    if (strlen($_buffer["xor"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            $_key = \OCA\NetBase\Vendor\phpseclib3\Common\Functions\Strings::shift($_buffer["xor"], ' . $block_size . ');
                            $_ciphertext.= $_block ^ $_key;
                        }
                    } else {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
