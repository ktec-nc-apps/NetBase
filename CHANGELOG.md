...
- **Device windows**: a web port in the device list opens that device's own interface in a window inside NetBase, fetched through the server. A link to a local address is useless from anywhere else; this is not a link but the page itself, delivered by the server that can reach it. Several windows can be open at once, and each can be moved, resized, reloaded or made to fill the screen.
  ...
  - A device interface built out of frames cannot load them while the window is held away from Nextcloud; NetBase says so plainly and offers to show it in full — one click in the window, said once and remembered for that device, so it opens correctly from then on. **Settings** lists the devices agreed to, and either that list or the window itself can take it back.
  - Whatever the window shows, the page can reach nothing but the proxy: its scripts, styles, images, forms and requests are pinned by policy to NetBase's own proxy path, so a device page cannot call a Nextcloud endpoint even when shown in full. It also stays sandboxed against navigating anything but itself — a device that tries to break out of frames cannot take the browser with it — and its own "replace everything" links fill the window instead.
